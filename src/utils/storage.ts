...
export const initializeData = (): AppData => {
    const storedData = localStorage.getItem('shipMaintenanceData');
    if (storedData) {
...
export const getData = (): AppData => {
    const data = localStorage.getItem('shipMaintenanceData');
...
export const addNotification = (message: string, type: string): void => {
    const data = getData();
